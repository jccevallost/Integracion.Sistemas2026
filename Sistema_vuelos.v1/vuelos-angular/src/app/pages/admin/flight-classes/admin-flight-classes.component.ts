...
type Row = { id: string; flightId: string; cabinClass: string; basePrice: number; availableSeats: number; flight?: { flightNumber: string; originAirportIata?: string; destinationAirportIata?: string } };
...
export class AdminFlightClassesComponent implements OnInit {
  ...
  cols = [
    { key: 'flight', label: 'Vuelo', render: (r: Row) => r.flight ? `${r.flight.flightNumber} (${r.flight.originAirportIata ?? '?'} → ${r.flight.destinationAirportIata ?? '?'})` : '—' },
